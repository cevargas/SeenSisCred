...
class Vendas extends CI_Controller {
    ...
    public function salvar() {
        ...
        if ($this->form_validation->run() === TRUE) {
			...
			//salva vendas
			$id = $this->Vendas_model->save($data);			
			
			$prodmap = array_map(null, $this->input->post('produtos'), $this->input->post('quantidade'));
			
			foreach ($prodmap as $pm) {
				if (count($pm) > 0) {	
					$dataItens = array();					
					$produto = $this->Produtos_model->getProduto($pm[0]);					
					$dataItens['id_venda'] = $id;
					$dataItens['id_produto'] = $produto['id'];
					$dataItens['valor'] = $produto['valor'];
					$dataItens['quantidade'] = $pm[1];
